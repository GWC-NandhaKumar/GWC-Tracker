...
//     .then(data => {
//         // Step 2: Extract IDs from the data
//         const ids = data.map(doc => doc.id).join(',');

//         // Step 3: Delete documents in bulk using the extracted IDs
//         return domo.delete(`/domo/datastores/v1/collections/ProjectTrackerDB-NewSet/documents/bulk?ids=${ids}`);
//     })
//     .then(deleteResponse => {
//         console.log('Deleted Documents:', deleteResponse);
//     })
//     .catch(error => {
...
$(document).ready(function populate() {
  ...
  const fetchData = () => {
    const url = id
      ? `/domo/datastores/v1/collections/ProjectTrackerDB-NewSet/documents/${id}`
      : "/domo/datastores/v1/collections/ProjectTrackerDB-NewSet/documents/";
    domo
      .get(url)
      .then((data) => {
        ...
        projects.forEach((project) => {
          ...
          if (project.content.ProjectArchieveStatus !== "Yes") {
            ...
            row.innerHTML = `
         <td class="text-left hover:text-blue-600">
                            <a href="./project_detail.html?${project.id}">${
              project.content.projectName
            }</a>
                        </td>
                        <td class="text-left">${
                          project.content.gwcAccountManager
                        }</td>
                        <td class="text-left">${
                          project.content.gwcProjectManager
                        }</td>
                        <td class="text-sm text-center ">${
                          project.content.projectLeadDate
                        }</td>
                        <td class="text-left cursor-pointer" onclick='updateStage(this, "PreSalesData", "${
                          project.id
                        }")'>
                            ${getStageStatus(project.content.PreSalesData)}
                        </td>
                        <td class="text-left cursor-pointer" onclick='updateStage(this, "ProjectInitiationData", "${
                          project.id
                        }", ${JSON.stringify(project.content.PreSalesData)})'>
                            ${getStageStatus(
                              project.content.ProjectInitiationData
                            )}
                        </td>
                        <td class="text-left cursor-pointer" onclick='updateStage(this, "ProjectExecutionData", "${
                          project.id
                        }", ${JSON.stringify(
              project.content.ProjectInitiationData
            )})'>
                            ${getStageStatus(
                              project.content.ProjectExecutionData
                            )}
                        </td>
                        <td class="text-left cursor-pointer" onclick='updateStage(this, "ProjectClosureData", "${
                          project.id
                        }", ${JSON.stringify(
              project.content.ProjectExecutionData
            )})'>
                            ${getStageStatus(
                              project.content.ProjectClosureData
                            )}
                        </td>
                        <td class="text-center">${capitalizeFirstLetter(
                          project.content.innovationProject
                        )}</td>
                        <td class="text-center">
                            <a href="./editproject.html?${
                              project.id
                            }" class="text-blue-600 hover:text-blue-800">
                                <i class="fas fa-edit"></i>
                            </a>
                            <button class="delete-btn text-red-600 hover:text-red-800 ml-2" onclick='showDeleteModal("${
                              project.id
                            }")'>
...
function updateCellColor(cell, stageData) {
  try {
    const status = stageData
      ? JSON.parse(stageData).OverallStageStatus
      : "Not Started";
    if (status === "Completed") {
      cell.classList.add("bg-[#4474c4]");
    } else if (status === "Active") {
      cell.classList.add("bg-[#22C55E]");
    } else {
...
function addPOCDeveloper() {
  ...
  developerElement.classList.add(
    "flex",
    "space-x-4",
    "mt-2",
    "input-container"
...
function autocomplete(inputElement) {
  ...
  const listContainer =
    inputElement.parentNode.querySelector(".autocomplete-list");
  listContainer.innerHTML = "";

  const suggestions = developerData.filter((dev) =>
    dev.emp_name.toLowerCase().includes(inputValue)
  );
...
function handleRadioChange(event, id, stage) {
  ...
  if (!yesComment || !noComment) {
    console.error(
      `Comment elements with IDs '#yes-comment-${id}' or '#no-comment-${id}' not found within form.`
    );
    return;
  }
...
function PresalescollectFormData() {
  ...
  let leaddate = document.getElementById("project_leaddate").value;
  ...
  console.log("lead date for presales", leaddate);
  let daysFromLeadToCurrent = getDaysFromLeadToCurrent(leaddate);
  ...
  const checklistElements =
    checklistContainer.querySelectorAll(".checklist-item");

  checklistElements.forEach((checklistElement) => {
    const id = checklistElement.getAttribute("data-id");
    const engagement =
      form.querySelector(`input[name="engagement-${id}"]:checked`)?.value || "";
    const comment =
      checklistElement.querySelector(`#yes-comment-${id} input`)?.value ||
      checklistElement.querySelector(`#no-comment-${id} input`)?.value ||
      "";
  ...
  domo
    ...
    .then((existingData) => {
      ...
      const stage2check = JSON.parse(
        updatedDocument.content.ProjectInitiationData
      );
      const stage3check = JSON.parse(
        updatedDocument.content.ProjectExecutionData
      );
      const stage4check = JSON.parse(
        updatedDocument.content.ProjectClosureData
      );

      if (
        stage2check.OverallStageStatus === "Completed" &&
        OverallStageStatus === "Completed" &&
        stage3check.OverallStageStatus === "Completed" &&
        (stage4check.OverallStageStatus === "Completed") === "Completed"
      ) {
        if (OverallStageStatus === "Completed") {
          console.log(
            JSON.parse(updatedDocument.content.ProjectInitiationData)
          );
        ...
        if (OverallStageStatus === "Completed") {
          ...
          if (PresaleStatus === "Closed Lost") {
            ...
            updatedDocument.content.ProjectExecutionData =
              JSON.stringify(stage3data);
            updatedDocument.content.ProjectClosureData =
              JSON.stringify(stage4data);
          } else {
            const stage2data = JSON.parse(
              updatedDocument.content.ProjectInitiationData
            );
    ...
    .then((response) => {
      ...
      document.getElementById("ListProjectTable").classList.remove("hidden");
    })
    .catch((error) => {
  ...
  if (!previousStageData) {
    ...
    domo
      ...
      .then((data) => {
        console.log(data);
        PreSalesprefillForm(
          JSON.parse(data.content.PreSalesData),
          data.content.projectLeadDate,
          data.content.projectName
        );
        console.log(data.content);
      });
    document
      .getElementById("PreSaleclosePopup")
...
function PreSalesprefillForm(data, leaddate, projectName) {
  console.log("insidePrefill", data);
  console.log(developerData);
  document.getElementById(
    "presalestextcontent"
  ).textContent = `Project Presales Stage - ${projectName}`;

  const form = document.querySelector("form.stage1");

  form.querySelector("#project_leaddate").value = leaddate;
  console.log(form.querySelector("#project_leaddate").value, leaddate);
  ...
  comments.forEach((comment) => {
    addComment();
    const lastCommentElement = form.querySelector(
  ...
  developers.forEach((developer) => {
    addPOCDeveloper();
    const lastDeveloperElement = form.querySelector(
      "#POCDevelopersContainer > div:last-child"
    );
    if (lastDeveloperElement) {
      const nameInput =
        lastDeveloperElement.querySelector('input[type="text"]');
      if (nameInput) nameInput.value = developer.name;
      const emailInput = lastDeveloperElement.querySelector(
        'input[type="email"]'
      );
  ...
  checklist.forEach((item) => {
    ...
    const engagementInput = form.querySelector(
      `input[name="engagement-${id}"][value="${engagement}"]`
    );
    if (engagementInput) {
...
function PresalesFormrenderChecklistItems() {
  ...
  checklistItems.forEach((item, index) => {
    ...
    checklistElement.classList.add(
      "checklist-item",
      "p-4",
      "bg-white",
      "rounded-md",
            ...
            <div class="flex space-x-4 mb-2">
                <label class="flex items-center">
                    <input type="radio" name="engagement-${
                      index + 1
                    }" value="yes" onchange="handleRadioChange(event, ${
      index + 1
    },'stage1')" class="mr-2"> Yes
                </label>
                <label class="flex items-center">
                    <input type="radio" name="engagement-${
                      index + 1
                    }" value="no" onchange="handleRadioChange(event, ${
      index + 1
    },'stage1')" class="mr-2"> No
                </label>
            </div>
...
function ProjectInitiationForm(projectId) {
  ...
  domo
    ...
    .then((data) => {
      console.log(data);
      let presale_completeddate = JSON.parse(
        data.content.PreSalesData
      ).completedDate;
      console.log(presale_completeddate);
      ProjectInitiationprefillForm(
        data.content.ProjectInitiationData,
        projectId,
        presale_completeddate,
        data.content.projectName
      );
      console.log(data.content);
    });
...
function ProjectInitiationcollectFormData() {
  ...
  let overallStageStatus =
    form.elements["ProjectinitationOverallStageStatus"].value;
  ProjectInitiationFormData.OverallStageStatus = overallStageStatus;

  ProjectInitiationFormData.Timeline.value =
    form.elements["TimelineValue"].value;
  ProjectInitiationFormData.Timeline.unit = form.elements["TimelineUnit"].value;

  ProjectInitiationFormData.ProposedStartDate =
    form.elements["ProposedStartDate"].value;
  ProjectInitiationFormData.ProposedEndDate =
    form.elements["ProposedEndDate"].value;
  ...
  ProjectInitiationFormData.NumberOfResources =
    ProjectInitiationFormData.resources.length;
  ...
  let leaddateValue = form.querySelector("#project_leaddate").value;
  console.log(leaddateValue);
  let daysFromLeadToCurrent = getDaysFromLeadToCurrent(leaddateValue);
  console.log(leaddateValue);
  console.log(daysFromLeadToCurrent);

  ProjectInitiationFormData.completedDate = new Date()
    .toISOString()
    .split("T")[0];
  ProjectInitiationFormData.daysFromLeadToCurrent = daysFromLeadToCurrent;
  console.log(ProjectInitiationFormData.daysFromLeadToCurrent);

  let checklistItems = form.querySelectorAll(".checklist-item");
  checklistItems.forEach((item) => {
    let checklistId = item.dataset.id;
    let engagement = item.querySelector('input[type="radio"]:checked')
  ...
  domo
    ...
    .then((existingData) => {
      ...
      if (
        stage1check.OverallStageStatus === "Completed" &&
        overallStageStatus === "Completed" &&
        stage3check.OverallStageStatus === "Completed" &&
        (stage4check.OverallStageStatus === "Completed") === "Completed"
      ) {
        if (overallStageStatus === "Completed") {
          ...
          updatedDocument.content.ProjectInitiationData = JSON.stringify(
    ...
    .then((response) => {
      console.log("Document updated successfully:", response);
      showNotification(
        "Project Initiation Stage Updated Successfully.",
        "green"
      );
      form.reset();
      document.getElementById("ProjectInitiationPopUp").classList.add("hidden");
      document.getElementById("ListProjectTable").classList.remove("hidden");
    })
    .catch((error) => {
      console.error("Error updating document:", error);
...
function ProjectInitiationprefillForm(dataa, projectId, leaddate, projectName) {
  console.log("Prefill function called with data:", dataa, projectId);
  const data = JSON.parse(dataa);
  document.getElementById(
    "initiationtextcontent"
  ).textContent = `Project Initiation Stage - ${projectName}`;
  ...
  form.querySelector("#ProjectinitationOverallStageStatus").value =
    data.OverallStageStatus || "Active";
  ...
  if (Array.isArray(comments) && comments.length > 0) {
    comments.forEach((comment) => {
      ...
      if (lastCommentElement) {
        const dateInput =
          lastCommentElement.querySelector('input[type="date"]');
        if (dateInput) dateInput.value = comment.date;
        const textInput =
          lastCommentElement.querySelector('input[type="text"]');
        if (textInput) textInput.value = comment.comment;
      }
    });
  }
  ...
  if (Array.isArray(checklist) && checklist.length > 0) {
    checklist.forEach((item) => {
      ...
      const comment = item.comment;
      const engagementInput = form.querySelector(
        `input[name="stage2-${id}"][value="${engagement}"]`
      );
      if (engagementInput) {
        engagementInput.checked = true;
        if (engagement === "yes") {
...
function ProjectExecutionForm(projectId) {
  ...
  domo
    ...
    .then((data) => {
      console.log(data);
      let projectInitiation_completeddate = JSON.parse(
        data.content.ProjectInitiationData
      ).completedDate;
      console.log(projectInitiation_completeddate);
      ProjectExecutionprefillForm(
        data.content.ProjectExecutionData,
        projectId,
        projectInitiation_completeddate,
        data.content.projectName
      );
      console.log(data.content);
    });
...
function ProjectExecutionprefillForm(dataa, projectId, leaddate, projectName) {
  console.log("Inside ProjectExecutionprefillForm");
  const data = JSON.parse(dataa);
  document.getElementById(
    "Executiontextcontent"
  ).textContent = `Project Execution Stage - ${projectName}`;
  ...
  form.querySelector("#ProjectExecutionOverallStageStatus").value =
    data.OverallStageStatus;
  ...
  checklist.forEach((item) => {
    ...
    const comment = item.comment;
    const engagementInput = form.querySelector(
      `input[name="stage3-${id}"][value="${engagement}"]`
    );
    if (engagementInput) {
      engagementInput.checked = true;
      if (engagement === "yes") {
...
function ProjectExecutionCollectFromData() {
  ...
  let overallStageStatus =
    form.elements["ProjectExecutionOverallStageStatus"].value;
  ...
  ProjectExecutionFormData.ActualStartDate =
    form.elements["ActualStartDate"].value;
  ProjectExecutionFormData.ActualEndDate = form.elements["ActualEndDate"].value;

  ProjectExecutionFormData.PercentComplete =
    form.elements["PercentComplete"].value;
  ...
  ProjectExecutionFormData.completedDate = new Date()
    .toISOString()
    .split("T")[0];
  ...
  domo
    ...
    .then((existingData) => {
      ...
      if (
        ...
        if (overallStageStatus === "Completed") {
          ...
          updatedDocument.content.ProjectExecutionData = JSON.stringify(
            ProjectExecutionFormData
          );
          return domo.put(
            "/domo/datastores/v1/collections/ProjectTrackerDB-NewSet/documents/" +
              projectId,
    ...
    .then((response) => {
      showNotification(
        "Project Execution Stage Updated Successfully.",
        "green"
      );
      form.reset();
      console.log("Document updated successfully:", response);
      document.getElementById("ProjectExecutionPopUp").classList.add("hidden");
      document.getElementById("ListProjectTable").classList.remove("hidden");
    })
    .catch((error) => {
      showNotification("Error updating document", "red");
...
function ProjectClosureForm(projectId) {
  ...
  domo
    ...
    .then((data) => {
      console.log(data);
      let ProjectExecutionData_completeddate = JSON.parse(
        data.content.ProjectExecutionData
      ).completedDate;
      console.log(ProjectExecutionData_completeddate);
      ProjectClosureprefillForm(
        data.content.ProjectClosureData,
        projectId,
        ProjectExecutionData_completeddate,
        data.content.projectName
      );
      console.log(data.content);
    });
...
function ProjectClosureprefillForm(dataa, projectId, leaddate, projectName) {
  console.log("Inside ProjectClosureformprefillForm");
  const data = JSON.parse(dataa);
  const form = document.querySelector("#ProjectClosureform");
  document.getElementById(
    "Closuretextcontent"
  ).textContent = `Project Closure Stage - ${projectName}`;
  if (!form) {
  ...
  form.querySelector("#ProjectClosureOverallStageStatus").value =
    data.OverallStageStatus;
  form.querySelector("#project_leaddate").value = leaddate;
  ...
  comments.forEach((comment) => {
    ...
    if (lastCommentElement) {
      ...
      if (dateInput) {
        dateInput.value = comment.date;
        dateInput.class =
          "border border-gray-200 outline-[#6E2F8E] mt-1  block w-[30%] text-2xl p-2 h-auto rounded-md   sm:text-sm";
      }
      const textInput = lastCommentElement.querySelector('input[type="text"]');
      if (textInput) {
        textInput.value = comment.comment;
        textInput.class =
          "border border-gray-200 outline-[#6E2F8E] mt-1  block w-[70%] text-2xl p-2 h-auto rounded-md   sm:text-sm";
      }
    }
  });
  ...
  checklist.forEach((item) => {
    ...
    const comment = item.comment;
    const engagementInput = form.querySelector(
      `input[name="stage4-${id}"][value="${engagement}"]`
    );
    if (engagementInput) {
      engagementInput.checked = true;
      if (engagement === "yes") {
...
function ProjectClosureCollectFromData() {
  ...
  let overallStageStatus =
    form.elements["ProjectClosureOverallStageStatus"].value;
  ProjectClosureFormData.OverallStageStatus = overallStageStatus;

  ProjectClosureFormData.ProjectDocumentation =
    form.elements["ProjectDocumentation"].value;
  ProjectClosureFormData.ClientAcceptance =
    form.elements["ClientAcceptance"].value;

  ProjectClosureFormData.PaymentReceived =
    form.elements["PaymentReceived"].value;
  ...
  domo
    ...
    .then((existingData) => {
      ...
      if (
        ...
        if (overallStageStatus === "Completed") {
          ...
          updatedDocument.content.ProjectClosureData = JSON.stringify(
            ProjectClosureFormData
          );
          return domo.put(
            "/domo/datastores/v1/collections/ProjectTrackerDB-NewSet/documents/" +
              projectId,
    ...
    .then((response) => {
      ...
      form.reset();
      console.log("Document updated successfully:", response);
      document.getElementById("ProjectClosurePopUp").classList.add("hidden");
      document.getElementById("ListProjectTable").classList.remove("hidden");
    })
    .catch((error) => {
      showNotification("Error updating document", "red");
...
function closeProject(projectId) {
  domo
    .get(
      `/domo/datastores/v1/collections/ProjectTrackerDB-NewSet/documents/${projectId}`
    )
    .then((data) => {
      const project = data;
...
function showDeleteModal(id) {
  ...
  document.getElementById("confirmDelete").onclick = function () {
    ...
    deleteProject(id); // Call delete function with project ID
  ...
  document.getElementById("cancelDelete").onclick = function () {
    ...
    document.getElementById("deleteModal").classList.add("hidden"); // Simply hide modal on cancel
  };
}
  ...
  domo
    ...
    .then((result) => {
      if (result.status === 200) {
        ...
        fetchData(); // Assuming fetchData() refreshes your project data
        window.location.reload();
      } else {
        console.error(
          `Failed to delete document ${id} from collection. Status: ${result.status}`
        );
        window.location.reload();
      }
    })
